...
def contains_double(number):
    return any(has_double.findall(str(number)))
...
def contains_exact_double(number):
    ...
    disallowed_multiples = [i * str(number) for i in range(3, 6) for number in '0123456789']
    for disallowed in disallowed_multiples:
